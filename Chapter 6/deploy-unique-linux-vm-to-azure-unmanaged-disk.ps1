...
.SYNOPSIS
This script will deploy a Linux VM into Azure on an unmanaged disk.
...
- The latest VM IMage version of Ubuntu Server 16.04 LTS is located and is what will be deployed on the VM.
...
The ID of the Azure Subscription you are deploying to, i.e. - 1e8f6ef4-93cf-48c9-a1f9-1436e81e6fec
...
.NOTES
Filename:   deploy-unique-linux-vm-to-azure-unmanaged-disk.ps1
Author:     Ryan Irujo (https://github.com/starkfell)
Language:   PowerShell 5.0

Syntax:           ./deploy-unique-linux-vm-to-azure-unmanaged-disk.ps1 `
...
Example:          ./deploy-unique-linux-vm-to-azure-unmanaged-disk.ps1 `
                  ...
                  -ResourceGroupName iaas-linux-vm `
...
# Retrieving the latest version of Ubuntu Server 16.04 LTS that is avialable in the Location being deployed to.
...
{
    Write-Output "Successfully retrieved the lastest VM Image for Ubuntu Server 16.04 LTS."
}

If ($RetrieveVMImageFail)
{
	Write-Error -Message "Failed to retrieve the lastest VM Image for Ubuntu Server 16.04 LTSr."
    exit 2
}
